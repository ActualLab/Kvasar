...
This document describes the coding conventions used across ActualLab projects (ActualLab.Fusion,
ActualLab.Kvasar, etc.) that differ from standard .NET conventions. It is adopted verbatim by
ActualLab.Kvasar; the few Fusion-only helpers it mentions (`FilePath`, `TaskCompletionSourceExt`,
`SilentAwait`/`ResultAwait`, `LogFor`) live in ActualLab.Core, which Kvasar intentionally does not
reference — see `AGENTS-Source.md` ("Zero dependencies") for that carve-out.
...
Search for `<NoWarn>` to see the list of disabled warnings (in `Directory.Build.props`).

See [`.editorconfig`](.editorconfig) for the complete list of silenced analyzer warnings.

<!-- ActualLab.Kvasar is a C#-only library; the Fusion style guide's TypeScript section
     has been dropped here. Refer to ActualLab.Fusion's CODING_STYLE.md if you need it. -->
